...
<div class="settings-container">
    ...
    <div class="settings-card">
        ...
        <form action="{{ route(auth()->user()->hasRole('Catering Staff') ? 'catering-staff.settings.update-profile' : (auth()->user()->hasRole('Inventory Personnel') ? 'inventory-personnel.settings.update-profile' : (auth()->user()->hasRole('Inventory Supervisor') ? 'inventory-supervisor.settings.update-profile' : (auth()->user()->hasRole('Security Staff') ? 'security-staff.settings.update-profile' : (auth()->user()->hasRole('Catering Incharge') ? 'catering-incharge.settings.update-profile' : (auth()->user()->hasRole('Ramp Dispatcher') ? 'ramp-dispatcher.settings.update-profile' : (auth()->user()->hasRole('Flight Purser') ? 'flight-purser.settings.update-profile' : (auth()->user()->hasRole('Flight Dispatcher') ? 'flight-dispatcher.settings.update-profile' : 'cabin-crew.settings.update-profile')))))))) }}" method="POST">
            @csrf
            @method('PUT')
    ...
    <div class="settings-card">
        ...
        <form action="{{ route(auth()->user()->hasRole('Catering Staff') ? 'catering-staff.settings.update-password' : (auth()->user()->hasRole('Inventory Personnel') ? 'inventory-personnel.settings.update-password' : (auth()->user()->hasRole('Inventory Supervisor') ? 'inventory-supervisor.settings.update-password' : (auth()->user()->hasRole('Security Staff') ? 'security-staff.settings.update-password' : (auth()->user()->hasRole('Catering Incharge') ? 'catering-incharge.settings.update-password' : (auth()->user()->hasRole('Ramp Dispatcher') ? 'ramp-dispatcher.settings.update-password' : (auth()->user()->hasRole('Flight Purser') ? 'flight-purser.settings.update-password' : (auth()->user()->hasRole('Flight Dispatcher') ? 'flight-dispatcher.settings.update-password' : 'cabin-crew.settings.update-password')))))))) }}" method="POST">
            @csrf
            @method('PUT')
    ...
    <div class="settings-card">
        ...
        <form action="{{ route(auth()->user()->hasRole('Catering Staff') ? 'catering-staff.settings.update-preferences' : (auth()->user()->hasRole('Inventory Personnel') ? 'inventory-personnel.settings.update-preferences' : (auth()->user()->hasRole('Inventory Supervisor') ? 'inventory-supervisor.settings.update-preferences' : (auth()->user()->hasRole('Security Staff') ? 'security-staff.settings.update-preferences' : (auth()->user()->hasRole('Catering Incharge') ? 'catering-incharge.settings.update-preferences' : (auth()->user()->hasRole('Ramp Dispatcher') ? 'ramp-dispatcher.settings.update-preferences' : (auth()->user()->hasRole('Flight Purser') ? 'flight-purser.settings.update-preferences' : (auth()->user()->hasRole('Flight Dispatcher') ? 'flight-dispatcher.settings.update-preferences' : 'cabin-crew.settings.update-preferences')))))))) }}" method="POST">
            @csrf
            @method('PUT')

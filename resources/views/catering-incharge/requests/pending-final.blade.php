...
@section('title', 'Final Approval - Catering Incharge')
...
<div style="padding: 32px; max-width: 1400px; margin: 0 auto;">
    ...
    <div style="margin-bottom: 32px;">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <div>
                <h1 style="font-size: 32px; font-weight: 700; color: #1a202c; margin: 0 0 8px 0;">Final Approval Required</h1>
                <p style="color: #718096; font-size: 16px; margin: 0;">Catering Staff has received items - give final approval to forward to Security</p>
            </div>
            <a href="{{ route('catering-incharge.dashboard') }}" style="background: #6c757d; color: white; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: 600;">← Back to Dashboard</a>
        </div>
    </div>
    ...
    <div style="background: white; border-radius: 16px; padding: 28px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);">
        <div style="margin-bottom: 20px;">
            <h3 style="font-size: 18px; font-weight: 600; color: #1a202c; margin: 0 0 4px 0;">{{ $requests->total() }} Requests Awaiting Final Approval</h3>
            <p style="font-size: 13px; color: #718096; margin: 0;">Items have been issued and received by Catering Staff - approve to forward to Security</p>
        </div>
            ...
            <table style="width: 100%; border-collapse: collapse;">
                <thead>
                    <tr style="border-bottom: 2px solid #e9ecef;">
                        ...
                        <th style="text-align: left; padding: 14px; font-size: 14px; font-weight: 600; color: #495057;">Received Date</th>
                        <th style="text-align: left; padding: 14px; font-size: 14px; font-weight: 600; color: #495057;">Status</th>
                        <th style="text-align: center; padding: 14px; font-size: 14px; font-weight: 600; color: #495057;">Actions</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr style="border-bottom: 1px solid #f1f3f5;">
                        ...
                        <td style="padding: 14px; font-size: 14px; color: #6c757d;">
                            ...
                            <div style="font-size: 12px; color: #6c757d; margin-top: 4px;">
                                @foreach($request->items->take(2) as $item)
                                    • {{ $item->product->name }} ({{ $item->quantity_requested }})<br>
                                @endforeach
                        ...
                        <td style="padding: 14px;">
                            <span style="background: #f59e0b; color: white; padding: 6px 14px; border-radius: 12px; font-size: 12px; font-weight: 600;">📦 Items Received</span>
                        </td>
                            ...
                            <div id="approve-{{ $request->id }}" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.6); z-index: 1000; align-items: center; justify-content: center; animation: fadeIn 0.2s;" onclick="this.style.display='none'">
                                <div style="background: white; padding: 28px; border-radius: 16px; max-width: 520px; width: 90%; box-shadow: 0 20px 60px rgba(0,0,0,0.3);" onclick="event.stopPropagation()">
                                    <div style="display: flex; align-items: center; margin-bottom: 20px;">
                                        ...
                                        <div>
                                            <h4 style="margin: 0; font-size: 20px; font-weight: 700; color: #1a202c;">Final Approval</h4>
                                            <p style="margin: 4px 0 0 0; font-size: 13px; color: #718096;">Request #{{ $request->id }}</p>
                                        </div>
                                    </div>
                                    <p style="color: #4a5568; font-size: 15px; line-height: 1.6; margin: 0 0 24px 0;">Give final approval for this request? This will forward it to Security for authentication.</p>
                                    <form action="{{ route('catering-incharge.requests.final-approve', $request) }}" method="POST">
                                        @csrf
    ...
    <div style="background: white; border-radius: 16px; padding: 60px 28px; text-align: center; box-shadow: 0 2px 8px rgba(0,0,0,0.08);">
        ...
        <h3 style="font-size: 20px; font-weight: 600; color: #1a202c; margin: 0 0 8px 0;">No Requests Awaiting Final Approval</h3>
        <p style="color: #718096; font-size: 14px; margin: 0;">All requests have been processed. New requests will appear here once Catering Staff receives items from Inventory.</p>
    </div>
    @endif
</div>
@endsection

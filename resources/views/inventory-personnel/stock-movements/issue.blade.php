...
@section('content')
    <div style="margin-bottom: 24px;">
        <h1 style="font-size: 28px; font-weight: 700; color: #111827; margin: 0;">Issue Stock to Flight</h1>
        <p style="color: #6b7280; font-size: 14px; margin-top: 4px;">Dispatch inventory items for flight operations</p>
    </div>

    <div
        style="background: #fffbeb; border-left: 4px solid #f59e0b; padding: 16px; border-radius: 12px; margin-bottom: 24px; color: #92400e; display: flex; align-items: flex-start; gap: 12px;">
        <span style="font-size: 20px;">⚠️</span>
        <p style="margin: 0; font-size: 14px; line-height: 1.5;">This action will decrease the product stock quantity in the
            <strong>Main Inventory</strong>. Please verify the issued items and flight reference before recording.</p>
    </div>

    <div class="card-atcl" style="padding: 32px; max-width: 800px;">
        <form method="POST" action="{{ route('inventory-personnel.stock-movements.store-issue') }}">
            @csrf

            <div style="margin-bottom: 24px;">
                <label class="label-atcl">Product <span style="color: #dc2626;">*</span></label>
                <select name="product_id" class="input-atcl" required>
                    <option value="">Select a product to issue</option>
                    @foreach($products as $product)
                        <option value="{{ $product->id }}" {{ old('product_id') == $product->id ? 'selected' : '' }}>
                            {{ $product->name }} (Available: {{ $product->quantity_in_stock }} {{ $product->unit_of_measure }})
                        </option>
                    @endforeach
                </select>
                @error('product_id')
            ...
            <div style="margin-bottom: 24px;">
                <label class="label-atcl">Reference Number (Flight/Dispatch) <span style="color: #dc2626;">*</span></label>
                <input type="text" name="reference_number" class="input-atcl" value="{{ old('reference_number') }}" required
                    placeholder="e.g., AT123, FLT-456, etc.">
            ...
            <div style="margin-bottom: 32px;">
                ...
                <textarea name="notes" class="input-atcl" style="min-height: 100px;"
                    placeholder="Describe any details about this stock issue...">{{ old('notes') }}</textarea>
                @error('notes')
                    <div style="color: #dc2626; font-size: 13px; margin-top: 6px;">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div
                ...
                <button type="submit" class="btn-atcl btn-atcl-primary"
                    style="min-width: 200px; background: #d97706; border-color: #d97706;">Confirm Issue</button>
            </div>
        </form>
    </div>
@endsection